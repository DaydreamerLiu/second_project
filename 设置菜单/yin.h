...
int j = 2;//初始化开始时音量条图片
int volume3 = 200;//开始音量大小改变
void initdeat();//初始化并改变
void volume1();//音量+
void volume2();//音量-
...
void volume1()
{
	...
	sprintf_s(cmd3, "setaudio  ./ggg.mp3 volume to %d", volume3);//拼接
	static MCIERROR ret;
	if (a > 0)//判断是那个音乐
	{
...
void music2()
{
	...
	char cmd2[100] = { 0 };//停置muaic1,3
	mciSendString("stop ./ppp.mp3", 0, 0, 0);
	mciSendString("stop ./ggg.mp3", 0, 0, 0);
	mciSendString("open  ./hhh.mp3 ", 0, 0, 0);//失败音效
...
void music3()
{
	...
	char cmd3[100] = { 0 };//停置muaic1,2
	mciSendString("stop ./hhh.mp3", 0, 0, 0);
	mciSendString("stop ./ppp.mp3", 0, 0, 0);
	mciSendString("open  ./ggg.mp3 ", 0, 0, 0);//失败音效
